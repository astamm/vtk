...
`inst/libs/x64/`, and records `VTK_LINK=shared` in `vtk.conf`. R
automatically adds `libs/x64/` to the Windows DLL search path when rvtk is
...
2. **DLL footprint will grow with VTK module requirements.**
   ...
   `inst/libs/x64/` for **all** downstream packages, which affects install
