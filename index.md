...
to link against it. It provides four utility functions —
...
and
[`VtkVersion()`](https://astamm.github.io/rvtk/reference/VtkVersion.md)
— that return the correct compiler and linker flags for however VTK was
found or installed on the current machine.
...
> prepends that directory to `PATH` via
> [`Sys.setenv()`](https://rdrr.io/r/base/Sys.setenv.html) when rvtk is
...
[`LdFlagsFile()`](https://astamm.github.io/rvtk/reference/LdFlagsFile.md)
(or [`LdFlags()`](https://astamm.github.io/rvtk/reference/LdFlags.md)):
...
When `modules = NULL` (the default), all available modules are linked,
...
[`use_rvtk()`](https://astamm.github.io/rvtk/reference/use_rvtk.md) from
the root of that package. It handles everything in one step:
...
2.  Write `src/Makevars` and `src/Makevars.win` that resolve VTK
    ...
    helper script.
3.  Write `tools/configure.R` that calls
...
[`LdFlagsFile()`](https://astamm.github.io/rvtk/reference/LdFlagsFile.md)
is preferred over
[`LdFlags()`](https://astamm.github.io/rvtk/reference/LdFlags.md) for
the linker flags because the full set of VTK `-l` flags can exceed the 8
191-character Windows command-line limit. `LdFlagsFile(path)` writes the
flags to a response file and returns the short `@path` token that GNU ld
and LLVM lld both support; on macOS and Linux the raw flags are returned
directly.
