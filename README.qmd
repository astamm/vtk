...
> **Windows — static libs only.** Only pure static archives (`.a`) are used
> on Windows; DLL-backed import libraries (`.dll.a`) are explicitly excluded.
> If a system or pacman-installed VTK provides only shared libraries the
> script falls through to the pre-built static fallback automatically.
...
::: {.callout-important title="Important notes for downstream package developers using Windows shared VTK"}
...
2. **DLL footprint will grow with VTK module requirements.**
   The current pre-built DLL set contains only the minimal modules needed by
   the rvtk package itself. Downstream packages that require additional VTK
   modules (e.g. `vtkRenderingOpenGL2`, `vtkFiltersCore`) must request that
   those modules be added to the pre-built DLL set by opening an issue on the
   rvtk repository. Each additional module increases the size of
   `inst/vtk-dlls/` for **all** downstream packages, which affects install
   time and CRAN size limits.
...
::: {.callout-tip title="Step 1 — `src/Makevars.in` (template, committed to version control)" icon="false" collapse="true"}
```makefile
...
::: {.callout-tip title="Step 2 — `configure`(`.win`)" icon="false" collapse="true"}
```sh
...
::: {.callout-tip title="Step 3 — `.gitignore` / `.Rbuildignore`" icon="false" collapse="true"}
...
::: {.callout-tip title="Step 4 - `cleanup`(`.win`)" icon="false" collapse="true"}
...
::: {.callout-tip title="Step 5 - Function import" icon="false" collapse="true"}
